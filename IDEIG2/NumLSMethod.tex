...
Legyen $\X$ egy valós, szeparábilis Hilbert-tér. Tekintsük az alábbi egyenletet:
\begin{equation}\label{eq:num:1}
Lu = Nu, 
\end{equation}
ahol $L \in \mathfrak{L}(\mathcal{X}\curvearrowright \mathcal{X})$ lineáris operátor, $N \in \mathcal{X} \rightarrow \mathcal{X}$ pedig egy nemlineáris operátor.
...
A következõ szekcióban taglaljuk, hogy hogyan állíthatjuk elõ egy bizonyos esetben a sajátfüggvényeket.
...
Legyen
$$
\mathcal{D}(L) := \left\{ \phi \in L^2[-1, 1] : \phi \in \mathfrak{C}^2, \alpha_{11} \phi(-1) + \alpha_{12} \phi'(-1)= 0, \alpha_{21} \phi(1) + \alpha_{22} \phi'(1)= 0 \right\},
$$
$L : \mathcal{D}(L) \rightarrow L^2[-1, 1]$ úgynevezett Sturm-Liouville operátor:
\begin{equation*}
L u = \frac{1}{w} ( -(p u')' + q u),
\end{equation*}
ahol
$$
p(x) > 0,\quad w(x) > 0\qquad(x \in [-1, 1]),
$$
$p, q, w$ adott analitikus függvények, és $\alpha_{ij} \in \mathbb{R} \; (i,j = 1, 2)$. \\
...
Még hasonlóképpen meg kell fogalmaznunk a peremfeltételeinket, ehhez felhasználjuk, hogy $\mathbf{t}$ egy vektorértékû függvény, a $[-1, 1]$ intervallumnak a végpontjain $$T_n(-1) = (-1)^n,\qquad\textrm{ill.}\qquad T_n(1) = 1.$$ Legyen
$$

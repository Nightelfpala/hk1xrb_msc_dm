...
\end{equation*}
$H_m$-ről látható, hogy lineáris, $\Dom L$-be képez, $H_m = (L_{\mkern 1mu \vrule height 2ex\mkern2mu S_m^\perp})^{-1}$, illetve $H_mLu = (I - P_m)u$. Az $S$-téren vett normája $\norm{H_m} = \frac{1}{\lambda_{m+1}}$, a $\mu$-szerinti normája $\mu(H_m) \leq \alpha \sigma(m)$, ahol $\sigma(m) = \left(\sum_{k = m+1}^\infty \left( \frac{\mu(\Phi_k)}{\lambda_k}\right)^2\right)^{1/2}$ (ez utóbbi Cauchy-Schwarz-egyenlőtlenséggel belátható). Ebből következik, hogy $\lim_{m \to \infty} \mu(H_m) = 0$.
